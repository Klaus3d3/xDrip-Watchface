 ...
 */
public class HmWearableListItemLayout extends LinearLayout
		implements HmWearableListView.OnCenterProximityListener {

	private CircledImageView mCircle;
	private HmTextView mName;
	...
	public HmWearableListItemLayout(Context context) {
		this(context, null);
	}

	public HmWearableListItemLayout(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}



	public HmWearableListItemLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		mSmallCircleRadius = getResources().getDimensionPixelSize(R.dimen.small_circle_radius2);
		mBigCircleRadius = getResources().getDimensionPixelSize(R.dimen.big_circle_radius2);
	...
	public void onCenterPosition(boolean animate) {
		if(animate) {
			...
			if (!mScalingUpAnimator.isRunning() && mCircle.getCircleRadius() != mBigCircleRadius) {
				...
				mCircle.animate().scaleX(NO_SCALE).scaleY(NO_SCALE).translationX(NO_X_TRANSLATION).alpha(NO_ALPHA).setDuration(500L);
				mName.animate().scaleX(NO_SCALE).scaleY(NO_SCALE).translationX(NO_X_TRANSLATION).alpha(NO_ALPHA).setDuration(500L);
			}
		} else {
	...
	public void onNonCenterPosition(boolean animate) {
		if(animate) {
			...
			if(!mScalingDownAnimator.isRunning() && mCircle.getCircleRadius() != mSmallCircleRadius) {
				...
				mCircle.animate().scaleX(SCALE).scaleY(SCALE).translationX(X_TRANSLATION).alpha(PARTIAL_ALPHA).setDuration(500L);
				mName.animate().scaleX(SCALE).scaleY(SCALE).translationX(X_TRANSLATION).alpha(PARTIAL_ALPHA).setDuration(500L);
